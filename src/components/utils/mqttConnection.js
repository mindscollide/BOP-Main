...
export const useMqttClient = ({
  ...
  const [subscribedTopics, setSubscribedTopics] = useState([]);
  const clientRef = useRef(null);
  const topicsToSubscribeRef = useRef([]);
  const randomString = useRef(secureRandomString());
  const onConnectedSetRef = useRef(false); // ✅ guard against reassigning onConnected
  const pingIntervalRef = useRef(null);    // ✅ keep-alive ping interval
  ...
  // ✅ Start keep-alive ping — sends a dummy publish every 30s to prevent broker timeout
  const startPing = useCallback(() => {
    stopPing();
    pingIntervalRef.current = setInterval(() => {
      if (clientRef.current?.isConnected()) {
        try {
          const pingMsg = new Paho.Message("ping");
          pingMsg.destinationName = "BOP_KEEPALIVE";
          pingMsg.qos = 0;
          pingMsg.retained = false;
          clientRef.current.send(pingMsg);
  ...
  const onConnectionLost = useCallback(
    (resObj) => {
      ...
      setIsConnected(false);
      setSubscribedTopics([]);
      stopPing(); // ✅ stop ping on disconnect
      onConnectionLostCallback?.(resObj);
    },
    [onConnectionLostCallback, stopPing]
  );
  ...
  const connectToMqtt = useCallback(
    ({ subscribeID, userID }) => {
      ...
      // ✅ Only set onConnected ONCE to avoid duplicate subscriptions
      if (!onConnectedSetRef.current) {
        clientRef.current.onConnected = () => {
          console.log("MQTT connected / reconnected");
          setIsConnected(true);
          subscribeToTopics(topicsToSubscribeRef.current);
          startPing(); // ✅ start keep-alive on every connect/reconnect
        };
        onConnectedSetRef.current = true;
      }
      ...
      try {
        clientRef.current.connect({
          ...
          keepAliveInterval: 30,  // ✅ lowered from 60 → 30s to prevent broker timeout
          reconnect: true,
          cleanSession: false,
          timeout: 30,
          userName: import.meta.env.VITE_MQTT_USERNAME,
    ...
    [onMessageArrived, onConnectionLost, subscribeToTopics, startPing, isBranch, isCorporate]
  );
  ...
  const disconnectMqtt = useCallback(() => {
    if (clientRef.current?.isConnected()) {
      ...
      setIsConnected(false);
      setSubscribedTopics([]);
      stopPing(); // ✅ stop ping on manual disconnect
      console.log("MQTT manually disconnected");
    }
  }, [stopPing]);
