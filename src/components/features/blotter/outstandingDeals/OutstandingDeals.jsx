...
const OutstandingDeals = () => {
  ...
  const blotterTransactionRFQExpired = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionRFQExpired
  );

  const blotterTransactionAssigned = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionAssigned
  );
  const blotterTransactionAdded = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionAdded
  );
  const blotterTransactionRFQQuoted = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionRFQQuoted
  );
  const blotterTransactionAccepted = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionAccepted
  );

  const blotterTranscationCancelled = useSelector(
    (state) => state.BlotterSlicer.BlotterTranscationCancelled
  );

  const blotterTransactionCancellationRequest = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionCancellationRequestData
  );

  const blotterTransactionRejected = useSelector(
    (state) => state.BlotterSlicer.BlotterTransactionRejected
  );

  const OutstandingTableData = useSelector(
    (state) => state.BlotterSlicer.OutstandingTableData
  );
  const OutstandingTableDataCount = useSelector(
    (state) => state.BlotterSlicer.OutstandingTableDataCount
  );
  ...
  console.log(
    ...
    "getBlotterOutstandingDatagetBlotterOutstandingData"
  );
  //local states
  ...
  useEffect(() => {
    try {
      if (
        getBlotterOutstandingData &&
        OutstandingTableData.length > 0 &&
        OutstandingTableDataCount !== 0
      ) {
        setHasReachedBottom(false);
        setBlotterdata(OutstandingTableData);
        setTotalRecords(OutstandingTableDataCount);
        setRow(OutstandingTableData.length);
