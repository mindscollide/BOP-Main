...
const TXNTreasurySummary = () => {
  ...
  useEffect(() => {
    ...
    if (hasReachedBottom) {
      setBlotterdata((prev) => [...prev, ...GlobalStateGetBlotterData.tnxSummary]);
  ...
  const handleSelectAll = (filterKey) => {
    setFilterStates((prev) => ({
      ...prev,
      [filterKey]: { ...prev[filterKey], selectedItems: FILTER_OPTIONS[filterKey] },
    }));
  };
  ...
  const handleTransactionAction = (transactionID, type) => {
    if (type === "Accepted") {
      dispatch(AcceptRFQTransaction({ PK_TransactionID: transactionID }, navigate));
    } else {
  ...
  const Treasurycolumns = [
    ...
    createFilterColumn("Amount", "AMOUNT2", "amount", 120, (text) => text.toFixed(2)),
