    ...
    const columns = [
        {
            ...
            render: (text, record, index) => {
                ...
                return (
                    <>
                        <span className={`${isExpanded ? 'expanded' : ''} mis-volumwise-value bg-none color-black tp-customer-hd roboto-13`}>
                            {text}
                            <span className="view-detail cursor-pointer">
                                <IconElement onClick={() => handleExpandClick(record)} iconClass={`icon-add-circle-fill fs-6 mx-1 ${index === 1 ? 'color-green' : 'color-blue'}`}></IconElement>
                            </span>
                        </span>
                        {isExpanded ?
                            <div className='d-grid'>
                                <span class="mis-volumwise-value bg-none color-black py-0 roboto-13">Import</span>
                                <span class="mis-volumwise-value bg-none color-black py-0 roboto-13">Export</span>
                            </div> : null}
                    </>)
            },
        },
        {
            ...
            render: (text, record, index) => {
                const isExpanded = expandedRowKeys.includes(record.key);
                return (
                    <span className={`${isExpanded ? 'expanded' : ''} roboto-13 mis-volumwise-value bg-none color-black`}>{text}</span>
                )
            },
        },
        {
            ...
            render: (text, record, index) => {
                ...
                return (
                    <>
                        <span className={`${isExpanded ? 'expanded' : ''} ${index === 1 ? 'mis-profitwise-value' : 'mis-volumwise-value'} roboto-13`}>
                            {text}
                        </span>
                        {isExpanded ?
                            <div className='d-grid'>
                                <span className={`${index === 1 ? 'mis-profitwise-value' : 'mis-volumwise-value'} bg-none py-0 roboto-13`}>{record.import}</span>
                                <span className={`${index === 1 ? 'mis-profitwise-value' : 'mis-volumwise-value'} bg-none py-0 roboto-13`}>{record.export}</span>
                            </div> : null}
                    </>)
            },
        },
                    ...
                    <div className="flex-fill px-3">
                        ...
                        <div className="expanded-row">
                            <div className="expanded-column first-column">
                                <span class="color-hd border-0 roboto-13">Total Profit (PKR)</span>
                            </div>
